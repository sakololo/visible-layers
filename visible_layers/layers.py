"""Layer extraction and import helpers."""
...
def import_layer_folder(layers_dir: str | Path, output_dir: str | Path) -> dict:
    ...
    for index, source_path in enumerate(source_paths):
        layer_image = Image.open(source_path).convert("RGBA")
        if canvas_size is None:
            canvas_size = layer_image.size
        if layer_image.size != canvas_size:
            raise ValueError(
                f"Layer size {layer_image.size} does not match canvas size {canvas_size}: {source_path}"
            )
    ...
    metadata = make_character_metadata(
        ...
        height,
        layer_records,
        source=str(layer_source_directory),
    )
    write_metadata(metadata, output_directory / "character.json")
    return metadata
